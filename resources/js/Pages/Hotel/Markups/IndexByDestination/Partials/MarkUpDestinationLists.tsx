...
import { MarkupDestinationProps, Paginate } from '@/types';
import { router } from '@inertiajs/react';
import { Edit, Eye, Plus, Search, Trash2 } from 'lucide-react';
import { useEffect, useState } from 'react';

export default function MarkUpDestinationLists() {
    const [search, setSearch] = useState('');
    const [loading, setLoading] = useState(false);
    const [page, setPage] = useState<Paginate<MarkupDestinationProps> | null>(
        null,
    );
    ...
    const [itemToDelete, setItemToDelete] =
        useState<MarkupDestinationProps | null>(null);
            ...
            const res =
                typeof urlOrPage === 'string'
                    ? await hotelMicroserviceApi.get(urlOrPage, {
                          params: { scope: 'dst', search: search },
                      })
            ...
            setPage({
                ...
                links: data.labels.map((l: any) => ({
                    url: l.label.includes('Previous')
                        ? data.has_prev
                            ? `/markup-rule/list?scope=dst&page=${data.page - 1}`
                            : null
                        : l.label.includes('Next')
                          ? data.has_next
                              ? `/markup-rule/list?scope=dst&page=${data.page + 1}`
                              : null
                          : `/markup-rule/list?scope=dst&page=${l.label}`,
                    label: l.label,
                    active: l.active,
                })),
                ...
                next_page_url: data.has_next
                    ? `/markup-rule/list?scope=dst&page=${data.page + 1}`
                    : null,
            } as Paginate<MarkupDestinationProps>);
        } catch (error) {
            console.error('Failed to fetch destination markups:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAction = (action: string, item: MarkupDestinationProps) => {
        if (action === 'view') {
            ...
            router.visit(route('hotel.markups.destination.edit', item.key));
        } else {
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            alert('Failed to delete destination markup.');
        } finally {
            setIsDeleting(false);
        }
    };
    ...
    return (
        <div className="mt-6">
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-[#E2E8F0]">
                <TableHeader
                    title="Destination Markup Rules"
                    subtitle="Manage your markup pricing for destinations."
                    searchValue={search}
                    ...
                    actions={[
                        {
                            ...
                            onClick: () =>
                                router.visit(
                                    route('hotel.markups.destination.create'),
                                ),
                        },
                    ]}
                    // dropdownLabel="Sort"
            ...
            <MarkUpDetailModal
                ...
                title="Destination Markup Details"
            />
            ...
            <ConfirmModal
                ...
                title="Delete Destination Markup"
                message={
                    <>
